...
class AllExerciseBySerie extends ConsumerWidget {
  const AllExerciseBySerie({super.key, required this.pageController, required this.bgPageController});
    ...
    return Stack(
      children: [
        PageView(
          ...
          children: exercises.map((e) => Image.network(
            e.exerciseImageUrl ?? '',
            fit: BoxFit.cover,
            errorBuilder: (_, __, ___) => const Center(
              child: Icon(Icons.image_not_supported_outlined, size: 48, color: Colors.white24),
            ),
          )).toList(),
        ),
         DecoratedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              Expanded(child:  ClipRRect(
                                      borderRadius: BorderRadius.only(topLeft: Radius.circular(AppSpacing.md), topRight: Radius.circular(AppSpacing.md)),
                   ...
                   child: Column(
                     children: [
                       ...
                       DecoratedBox(decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.only(topLeft: Radius.circular(AppSpacing.md), topRight: Radius.circular(AppSpacing.md)))
                        ,child: Padding(
                          padding: AppSpacing.md.top.add( (MediaQuery.paddingOf(context).bottom + AppSpacing.xs).bottom),
                          child: Column(
                            children: [
                              Padding(
                                  ...
                                  return Padding(
                                    ...
                                    child: DecoratedBox(
                                      ...
                                      child: Padding(
                                        ...
                                        SireBody(scheduleId: e.scheduleId, name: e.exerciseName, repiteType: e.metricType, series: e.sets),
                                        AddSerieButton(scheduleId: e.scheduleId, 
                                        
                                        onTapSerie: () {
                                          ref.read(serieDetailProvider(e.scheduleId).notifier).addSerie();
                                        },
                                        onCompleteAll: () {
                                          ref.read(serieDetailProvider(e.scheduleId).notifier).completeAll();
                                        },
                                        ),
                                                                  ],
                                                                  ),
