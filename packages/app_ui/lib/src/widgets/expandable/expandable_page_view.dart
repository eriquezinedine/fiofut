...
typedef ExpandableWidgetBuilder = Widget Function(BuildContext context, int index);

class ExpandablePageSizeInfo {
  final int index;
  final double size;

  const ExpandablePageSizeInfo({required this.index, required this.size});
}

typedef ExpandablePageSizeCallback = ValueChanged<ExpandablePageSizeInfo>;
...
class ExpandablePageView extends StatefulWidget {
  ...
  final ExpandableWidgetBuilder? itemBuilder;
  ...
  final ExpandablePageSizeCallback? onPageSizeChanged;
  ...
  ExpandablePageView.builder({
    ...
    required ExpandableWidgetBuilder itemBuilder,
    this.controller,
    this.onPageChanged,
    this.onPageSizeChanged,
    this.reverse = false,
...
class _SizeAnimationData {
  ...
  bool operator ==(Object other) =>
      ...
      other is _SizeAnimationData &&
          previousSize == other.previousSize &&
          currentSize == other.currentSize &&
          firstPageLoaded == other.firstPageLoaded;

  @override
  int get hashCode => Object.hash(previousSize, currentSize, firstPageLoaded);
}
...
class _ExpandablePageViewState extends State<ExpandablePageView> {
  ...
  late final ValueNotifier<_SizeAnimationData> _animationNotifier;
  bool _pendingPageSizeEmission = false;
  ...
  double _maxVisibleSizeForPage(int virtualPage) {
    ...
    final viewportFraction = _pageController.viewportFraction;
    final visibleOnEachSide = ((1 - viewportFraction) / (2 * viewportFraction)).round();
  ...
  void _notifyAnimation() {
    _animationNotifier.value = _SizeAnimationData(
      previousSize: _previousSize,
      currentSize: _currentSize,
      firstPageLoaded: _firstPageLoaded,
    );
  }
  ...
  void initState() {
    ...
    _animationNotifier = ValueNotifier(
      _SizeAnimationData(
        previousSize: _previousSize,
        currentSize: _currentSize,
        firstPageLoaded: _firstPageLoaded,
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<_SizeAnimationData>(
      valueListenable: _animationNotifier,
      child: _buildPageView(),
      builder: (context, data, child) {
        return TweenAnimationBuilder<double>(
          curve: widget.animationCurve,
          duration: data.firstPageLoaded
              ? widget.animationDuration
              : widget.animateFirstPage
                  ? widget.animationDuration
                  : Duration.zero,
          tween: Tween<double>(
            begin: data.previousSize,
            end: data.currentSize,
          ),
  ...
  void _reinitializeSizes() {
    ...
    final currentPageSize =
        _sizes.elementAtOrNull(realCurrentPage) ?? widget.estimatedPageSize;
    ...
    if (!widget.loop && realCurrentPage >= _sizes.length) {
      ...
      _previousPage =
          (_currentPage + differenceFromPreviousToCurrent).clamp(0, _sizes.length - 1);
    }
  ...
  void _updatePage() {
    ...
    if (_currentPage != newPage) {
      ...
      _notifyAnimation();
      _schedulePageSizeEmission();
    }
  }

  void _schedulePageSizeEmission() {
    if (_pendingPageSizeEmission || widget.onPageSizeChanged == null) return;
    _pendingPageSizeEmission = true;
    WidgetsBinding.instance.addPostFrameCallback(_checkScrollSettled);
  }

  void _checkScrollSettled(Duration _) {
    if (!mounted || _pageController.positions.length != 1) {
      _pendingPageSizeEmission = false;
      return;
    }
    final page = _pageController.page;
    if (page != null && (page - page.roundToDouble()).abs() < 0.01) {
      _pendingPageSizeEmission = false;
      final realIndex = _realIndex(_currentPage);
      widget.onPageSizeChanged?.call(
        ExpandablePageSizeInfo(
          index: realIndex,
          size: _sizes[realIndex],
        ),
      );
    } else {
      WidgetsBinding.instance.addPostFrameCallback(_checkScrollSettled);
  ...
  Widget _itemBuilder(BuildContext context, int virtualIndex) {
    ...
    return _SizeReportingChild(
      onSizeChange: (size) {
        ...
        _notifyAnimation();
        if (realIndex == _realIndex(_currentPage)) _schedulePageSizeEmission();
      },
      ...
      for (int i = 0; i < widget.children!.length; i++)
        _SizeReportingChild(
          onSizeChange: (size) {
            ...
            _notifyAnimation();
            if (i == _realIndex(_currentPage)) _schedulePageSizeEmission();
          },
          alignment: widget.alignment,
          scrollDirection: widget.scrollDirection,
...
class _SizeReportingChild extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return OverflowBox(
      ...
      minWidth: scrollDirection == Axis.vertical ? double.infinity : null,
